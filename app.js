...
app.controller('AppController', function( $filter, FileService, $timeout, $log ) {
    ...
    function Init(){
      ...
      for(var x = 0; x < 0xFF; x++){
        var byte = byteROR( { Mode : 'Accumulator' }, x);
        console.log( x.toString(2) + ' -> ' + byte.toString(2) + ' -> C: ' + vm.Registers.CarryFlag + ' -> Z: ' + vm.Registers.ZeroFlag + ' -> N: ' + vm.Registers.NegativeFlag );
      }
      */
   ...
   //   }
      ...
      if ( instruction != undefined){
        ...
        switch(instruction.Name){
          case 'ADC': // Add with Carry
          {
            ...
            alert( instruction.Name + ' not implemented' );
            break;
          }          
          ...
          case 'BCC': // Branch if Carry Clear
          {
            ...
            if ( vm.Registers.CarryFlag == 0 ){
              ...
              vm.Registers.ProgramCounter = addByteWithWrapAround( byte, vm.Registers.ProgramCounter );
    ...
    function setRunTo( address ){
      vm.runToAddress = $filter('hex')(address, 16);;
    }
